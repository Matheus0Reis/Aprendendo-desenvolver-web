{if empty($view)}
	...
	{/if}
	<tr><th>Assunto</th><th width="160">Destinatário</th><th width="140">Enviada em</th></tr>
	
		{foreach from=$mails item=arr key=id}
			<tr>
				<td><input name="id_{$id}" type="checkbox" /><a href="game.php?village={$village.id}&amp;screen=mail&amp;mode=out&amp;view={$id}">{$arr.subject|replace:"Re:":""}</a></td>
				<td><a href="game.php?village={$village.id}&amp;screen=info_player&amp;id={$arr.to_id}">{$arr.to_username|replace:"System":"SpeeDGameS"}</a>{if $arr.is_read==0} <span class="fonte5">(Nao Lido)</span>{else} <span class="fuck_menu">(lido)</span>{/if}</td>
				<td>{$arr.time}</td>
			</tr>
		{/foreach}
		{if count($mails)>0}
...
{else}
	{if empty($error)}
		...
		<tr><th width="140">Assunto:</th><th width="300">{$mail.subject|replace:"Re:":""}</th></tr>
		<tr><td>De?</td><td><a href="game.php?village={$village.id}&amp;screen=info_player&amp;id={$mail.from_id}">{$mail.from_username}</a></td></tr>
		<tr><td>Para:</td><td><a href="game.php?village={$village.id}&amp;screen=info_player&amp;id={$mail.to_id}">{$mail.to_username}</a></td></tr>
		<tr><td>Enviada em:</td><td>{$mail.time}</td></tr>
		</table>
		...
		<td align="center"><a href="game.php?village={$village.id}&amp;screen=mail&amp;mode=block&amp;action=block_id&amp;id={$mail.from_id}&amp;h={$hkey}&amp;mode=out">Bloquear Destinatário</a></td>
		</tr>
		</table><br />
	{/if}
{/if}