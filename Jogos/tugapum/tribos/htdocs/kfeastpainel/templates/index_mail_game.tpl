...
<form action="?screen=mail_game&action=view" method="post">
        Jogador 1:<input name="from" type="text" value="{php}echo stripslashes(urldecode($user_from));{/php}" />
           
        
        Jogador 2:<input name="to" type="text" value="{php}echo stripslashes(urldecode($user_to));{/php}"/><br>
Limite de Resultados <input name="limit" type="text" size="2" value="{php}echo stripslashes(urldecode($limit));{/php}"/>
...
<br>
Copyright 2010 by paulgerman :) Version: 3.2 <a href="?screen=mail_game&changelog=1">Changelog</a>
</form>
<br><br>
{php}
if($_GET["changelog"] == "1") {
...
<table class="vis" width="100%"> 
		...
		</td><th>Data:</td><td>{php}echo $mesaj["time_mesaj"][$key]{/php}</th></tr> 
		<tr><th>Remetente | ID:</td><td>{php}echo $mesaj["nume_from_display"][$key],' | ',$mesaj["id_from_display"][$key];{/php}
		</th><th>Destinatario | ID:</th><td>{php}echo $mesaj["nume_to_display"][$key],' | ',$mesaj["id_to_display"][$key];{/php}</th></tr> 
