{if empty($view)}
	...
	<tr><th>Assunto</th><th width="160">Remetente</th><th width="140">Recebido em</th></tr>
		...
		{foreach from=$mails item=arr key=id}
			<tr>
				<td><input name="id_{$id}" type="checkbox" /><a href="game.php?village={$village.id}&amp;screen=mail&amp;mode=in&amp;view={$id}">{$arr.subject|replace:"Re:":""|replace:"Willkommen bei":"Bem Vindo à tribo"}</a>{if $arr.is_read==0} <span class="fonte5">(novo)</span>{/if}{if $arr.is_answered==1} <span class="fuck_menu"> (respondido)</span>{/if}{if $arr.is_read!=0 and $arr.is_answered==0} <span class="fuck_menu">(lido)</span>{/if}</td>
...
{else}
	{if empty($error)}
		...
		<tr><th width="140">Assunto</th><th width="300">{$mail.subject|replace:"Re:":""|replace:"Willkommen bei":"Bem Vindo à tribo"}</th></tr>
