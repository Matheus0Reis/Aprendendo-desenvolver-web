{if empty($view)}
	...
	<tr><th>Assunto</th><th width="160">Remetente</th><th width="160">Destinatário</th><th width="140">Data</th></tr>
		...
		{foreach from=$mails item=arr key=id}
			<tr>
				<td><input name="id_{$id}" type="checkbox" /><a href="game.php?village={$village.id}&amp;screen=mail&amp;mode=arch&amp;view={$id}">{$arr.subject|replace:"Re:":""}</a></td>
				<td>{if $arr.from_id==-1}{$arr.from_username|replace:"System":"SpeeDWars.net"}{else}<a href="game.php?village={$village.id}&amp;screen=info_player&amp;id={$arr.from_id}">{$arr.from_username}{/if}</a></td>
				<td><a href="game.php?village={$village.id}&amp;screen=info_player&amp;id={$arr.to_id}">{$arr.to_username}</a></td>
				<td>{$arr.time}</td>
			</tr>
		{/foreach}
		{if count($mails)>0}
			<tr><th><input name="all" type="checkbox" onclick="selectAll(this.form, this.checked)">selecionar todos</th><th colspan="3"></th></tr>
		{/if}
	</table>
...
{else}
	{if empty($error)}
		...
		<tr><td>De:</td><td><a href="game.php?village={$village.id}&amp;screen=info_player&amp;id={$mail.from_id}">{$mail.from_username|replace:"System":"SpeedWars.net"}</a></td></tr>
		...
		{$mail.text|replace:"schrieb":"escreveu"}
		</td></tr>
		</table>
