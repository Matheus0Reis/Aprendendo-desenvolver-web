...
if (isset($_GET['action']) && $_GET['action'] == 'status') {
	$update = $db->query("UPDATE support_ticket SET status = 'closed' WHERE id = '$id'");
	header ("Location: ?screen=ticket_closed&mode=showticket&ticketID=$id");
}
					...
					<div class="span8">
						<h2>Ticket: <?php echo $ticket['title'] ?></h2>
							...
							<div style="margin-bottom: 20px">
								...
								<a class="btn btn-success btn-small" href="?screen=ticket_open&mode=showticket&ticketID=<?php echo $id ?>&amp;action=status">O problema foi resolvido</a>
							</div>				
...
<h2>Responder</h2>
<form action="?screen=ticket_open&amp;mode=showticket&amp;ticketID=<?php echo $id ?>&amp;action=insert" method="post" >
...
?>
			<div class="row">
				<div class="span9">
					...
					<p>Aqui você pode ver uma lista dos tickets de suporte abertos e responde-los.</p>
		
					<table class="table table-striped table-bordered">
						<tr>
							<th style="width: 30%">Título</th>
							<th style="width: 15%">Mundo</th>
						...
						<?php
							while ($array = $db->fetch($ticket)) {	
								if ($array['status'] != 'closed') {
									...
									echo '<tr>';
										echo '<td><a href="?screen=ticket_open&mode=showticket&ticketID='.$array['id'].'">'.$array['title'].'</a></td>';
										echo '<td>'.$world['name'].'</td>';
										echo '<td>'.$array['last_response'].'</td>';
										echo '<td>'.$author['username'].'</td>';
										if ($status == 'Resolvido') {
											echo '<td>'.$status.'<a href=?screen=ticket_open&action=close&ticketID='.$array['id'].'><br>Fecha-lo</a></td>';
										} else {
											echo '<td>'.$status.'</td>';							
										}
									echo '</tr>';
